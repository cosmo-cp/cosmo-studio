...
export class NodeSecretStore implements SecretStore {
    ...
    public encrypt(value: string): string {
        ...
        return [PAYLOAD_VERSION, iv.toString('base64'), authTag.toString('base64'), encrypted.toString('base64')].join(
            ':',
        );
    }
    ...
    public decrypt(value: string): string {
        ...
        const decipher = crypto.createDecipheriv('aes-256-gcm', this.getKey(), Buffer.from(encodedIv, 'base64'));
        decipher.setAuthTag(Buffer.from(encodedAuthTag, 'base64'));
        const decrypted = Buffer.concat([decipher.update(Buffer.from(encodedEncrypted, 'base64')), decipher.final()]);
